...
export function TopUpModal({ onClose }: { onClose: () => void }) {
  ...
  const buyTokens = useAction(api.wallet.buyTokens);

  const handleTopUp = async () => {
    if (!user || !address) return;

    setIsToppingUp(true);
    toast.loading('Processing top up, will take a moment...');
    try {
      ...
      <div className='bg-white rounded-lg shadow-xl max-w-md w-full mx-4'>
        <div className='p-6'>
          <div className='flex justify-between items-center mb-6'>
            <h2 className='text-xl font-semibold text-gray-900'>Top Up Your Account!</h2>
              ...
              <div className='mb-4'>
                ...
                <div className='relative'>
                  <input
                    ...
                    onChange={(e) => setAmount(Math.floor(Number(e.target.value) * 100) / 100)}
                  />
                  <div className='absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none'>
                    <span className='text-gray-500'>coins</span>
                  </div>
                </div>
                <div>={(amount * 0.01).toFixed(2)} SGD</div>
              </div>
              <div className='mb-6'>
                ...
                <div className='relative bg-gray-100 rounded-lg p-1'>
                  ...
                  <div
                    className={`absolute top-1 left-1 w-1/2 h-9 bg-white rounded-md shadow-sm transition-transform duration-200 ease-out transform ${paymentMethod === 'credit_card' ? 'translate-x-0' : 'translate-x-[calc(100%-0.5rem)]'}`}></div>
                </div>
              </div>
            </div>
          ) : (
